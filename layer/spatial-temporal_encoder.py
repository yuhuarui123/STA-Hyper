...
class TimeEncoder(nn.Module):
    ...
    这是一个可训练的编码器，用于将连续时间值映射为低维时间向量。
    ts的输入应该像[E，1]一样，所有时间间隔都是值。
...
class DistanceEncoderHSTLSTM(nn.Module):
    ...
    首先确定不同槽位仓的位置，并在不同槽位内进行线性插值将插槽嵌入为可训练参数。
    """
    ...
    def cal_inter(self, ld, hd, l, h, embed):
        """
        Calculate a linear interpolation.计算线性插值。
