...
def test_step(model, data, ks=(1, 5, 10, 20)):
    ...
    for k_ in ks:
        ...
        # accuracy[k_] = accuracy(label_, pred_, k_).cpu().detach().numpy().tolist()
        ...
        # logging.info(f"[Evaluating] accuracy@{k_} : {accuracy[k_]},\tNDCG@{k_} : {NDCGs[k_]},\tMAP@{k_} : {MAPs[k_]}")
