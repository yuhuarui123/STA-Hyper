...
from layer import (
    ...
    DistanceEncoderSTAN,
    DistanceEncoderSimple, CommonEmbedding
)
...
class My_Model(nn.Module):
    def __init__(self, cfg):
        ...
        # self.region_embed_dim = cfg.model_args.region_embed_dim
        # self.hotness_embed_dim = cfg.model_args.hotness_embed_dim
        ...
        # self.hotness2idx_dict = get_map_dict(osp.join(predataset.data_path, 'hotness2idx_dict.csv'))
        # self.region2idx_dict = get_map_dict(osp.join(predataset.data_path, 'region2idx_dict.csv'))
    ...
    def forward(self, data, label=None, mode='train'):
        ...
        # region_embedding_model = CommonEmbedding(len(self.region2idx_dict.keys()), self.region_embed_dim)
        # region_feature = region_embedding_model
        # hotness_embedding_model = CommonEmbedding(len(self.hotness2idx_dict.keys()), self.hotness_embed_dim)
        # hotness_feature = hotness_embedding_model
